...
/// [`Ptr`] is the result of merging [`std::rc::Rc`] and [`std::cell::RefCell`] in an effort to provide
/// casting capabilities between structs and traits, while also featuring circular referencing in Rust
pub struct Ptr<T>
where
    T: ?Sized,
{
...
where
    T: ?Sized,
{
    /// Casts any `Ptr<T>` to `Ptr<U>` if the cast is valid base on the vtable lookup table that is
    /// generated by the generator based on the type hierarchy
...
// WeakPtr - custom
//////////////////////////////////////////
/// [`WeakPtr`] is a companion type to [`Ptr`] to allow cyclic references that are memory leak free
/// by not claiming ownership of the contained value
...
where
    T: ?Sized,
{
    /// Creates a new [weak][`WeakPtr`] pointer to this value.
    ...
    /// Gets the number of [weak][`WeakPtr`] pointers to this value.
    #[inline]
    ...
    /// Returns true if there are no other [`Ptr`] or [weak][`WeakPtr`] pointers to
    /// this inner value.
    #[inline]
